...
class RAGASEvaluator:
    ...
    def run_full_eval(
        ...
        # run_full_eval only receives raw strings, not a QAPair object, so we
        # build a minimal one here. BenchmarkRunner.run swaps in the original
        # QAPair afterward to preserve full metadata (see Task 4).
        qa_pair = QAPair(
            question=question,
...
class LLMJudge:
    ...
    def _build_prompt(self, question: str, answer: str, rubric: dict[str, Any]) -> str:
        ...
        return (
            ...
            '{"scores": {"<criterion>": <1-5>, ...}, "reasoning": "<brief explanation>"}'
        )

    def _parse_judge_response(self, raw_response: str, rubric: dict[str, Any]) -> dict[str, Any]:
        # Try to locate a JSON object anywhere in the raw text (handles cases
        # where the judge wraps it in prose or ```json fences).
            ...
            try:
                ...
                if isinstance(raw_scores, dict) and raw_scores:
                    ...
                    for criterion, value in raw_scores.items():
                        value = float(value)
                        # Judge may return a 1-5 scale or an already-normalized
                        # 0-1 score; normalize the former, clamp the latter.
                        if value > 1.0:
                            value = (value - 1.0) / 4.0
                        normalized[criterion] = max(0.0, min(1.0, value))
                    return {"scores": normalized, "reasoning": reasoning}
            except (json.JSONDecodeError, TypeError, ValueError):
                pass

        # Fallback: unparseable response -> default 0.5 per criterion.
        return {
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        def entry_mean(entry: dict[str, Any]) -> float:
            scores = entry.get("scores", {})
            if not scores:
                return 0.0
            return sum(scores.values()) / len(scores)

        entry_means = [entry_mean(e) for e in scores_batch]
        overall_avg = sum(entry_means) / len(entry_means)

        # Positional bias: compare responses scored in the "first" slot
        # (even indices) vs. the "second" slot (odd indices) of each pair.
        first_position = entry_means[0::2]
        second_position = entry_means[1::2]
        positional_bias = False
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Only pass retrieval contexts through when the pair actually has
            # some — an empty list would otherwise trigger context_recall/
            # context_precision to compute (and report) a spurious 0.0 for
            # QA pairs that never had a retriever attached.
            ...
            # run_full_eval builds its own minimal QAPair internally (it only
            # receives raw strings), so swap in the original pair here to
            # preserve full metadata (difficulty, category, source_docs, etc.)
            result.qa_pair = pair
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest_metrics = [name for name, value in scores.items() if value == min_score]

        # A tie among the three lowest scores means no single metric stands
        # out as the clear culprit -> treat as a systemic/multi-cause failure.
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            suggested_fix = suggestions[i] if i < len(suggestions) else "Review manually"
            rows.append(
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            ...
            suggestions.append(
                "Strengthen faithfulness guardrails — require answer claims to cite retrieved context"
            )
        if categories.get("irrelevant"):
            suggestions.append(
                "Clarify or restructure prompts to keep the agent focused on the actual question"
            )
        if categories.get("incomplete"):
            suggestions.append(
                "Increase chunk size or context window in the RAG pipeline to reduce fragmentation"
            )
            suggestions.append(
                "Add few-shot examples showing complete answers to improve completeness"
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Improve intent detection so the agent recognizes what the question is actually asking"
            )
        if categories.get("refusal"):
            suggestions.append(
                "Loosen overly strict guardrails causing valid questions to be refused"
            )

        # Fallback so the list always has at least 3 items when there are
        # any failures at all, even if categorization above yields fewer.
        general_suggestions = [
            "Expand the golden dataset with more examples covering these failure patterns",
            "Add regression tests to the CI/CD quality gate for this failure category",
            "Review failing cases manually using the 5 Whys method to confirm root cause",
        ]
